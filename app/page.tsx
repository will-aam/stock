...
export default function InventorySystem() {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      const mockProducts = [
        { id: 1, codigo_produto: "113639", descricao: "AGUA H2O LIMONETO 500ML", saldo_estoque: 50 },
        { id: 2, codigo_produto: "113640", descricao: "REFRIGERANTE COLA 350ML", saldo_estoque: 30 },
        { id: 3, codigo_produto: "113641", descricao: "SUCO LARANJA 1L", saldo_estoque: 25 },
      ]

      const mockBarCodes = [
        { codigo_de_barras: "7892840812850", produto_id: 1, produto: mockProducts[0] },
        { codigo_de_barras: "7892840812851", produto_id: 2, produto: mockProducts[1] },
        { codigo_de_barras: "7892840812852", produto_id: 3, produto: mockProducts[2] },
      ]
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardDescription>{isRegistering ? "Criar nova conta" : "Faça login para continuar"}</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Sistema de Estoque</h1>
                ...
                <CardContent>
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {productCounts.length === 0 ? (
                      <p className="text-gray-500 dark:text-gray-400 text-center py-8">Nenhum produto contado ainda</p>
                    ) : (
              ...
              <CardContent>
                {inventoryHistory.length === 0 ? (
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                    ...
                    <p className="text-sm">As contagens aparecerão aqui após serem finalizadas</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {inventoryHistory.map((history) => (
                      <div key={history.id} className="p-4 border dark:border-gray-700 rounded-lg">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="font-medium">{new Date(history.data_contagem).toLocaleString("pt-BR")}</p>
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              {history.usuario_email} •{" "}
                              {locations.find((l) => l.value === history.local_estoque)?.label}
                            </p>
                          </div>
                          <div className="text-right">
                            <Badge variant={history.status === "concluida" ? "default" : "secondary"}>
                              {history.status === "concluida" ? <CheckCircle className="h-3 w-3 mr-1" /> : null}
                              {history.status}
                            </Badge>
                            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{history.total_itens} itens</p>
                          </div>
                        </div>
                      </div>
                    ))}
